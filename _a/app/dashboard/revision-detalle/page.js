  ...
  const fetchData = useCallback(async () => {
    ...
    while (hasMore) {
      const { data, error } = await supabase
        .from("jira_tickets")
        .select("jira_key, summary, description, status, issue_type, sprint, story_points, assignee_email, reporter_email, parent_key, created_at, updated_at, synced_at, priority, labels")
        .is("deleted_at", null)
            ...
            <div className="hidden lg:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gray-50 border border-gray-200 text-xs text-gray-500">
              ...
              <span>
                ...
                <strong className="text-gray-700">
                  {lastUpdated.toLocaleDateString("es-PE", { day: "2-digit", month: "2-digit", year: "numeric" })}{" "}
